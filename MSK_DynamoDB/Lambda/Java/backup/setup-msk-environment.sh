...
# It should be downloaded and executed by the EC2 instance's UserData script

# Parameters (these will be passed as arguments)
STACK_NAME=$1
KAFKA_TOPIC_NAME=$2
GITHUB_REPO_URL=$3
AWS_REGION=$4
...
cat > /home/ec2-user/project/kafka-producer/build-and-run.sh << 'EOF'
#!/bin/bash
...
chmod +x /home/ec2-user/project/kafka-producer/build-and-run.sh

# Create SAM config file
mkdir -p /home/ec2-user/project/msk-dynamodb-processor
cat > /home/ec2-user/project/msk-dynamodb-processor/samconfig.toml << EOF
version = 0.1
[default]
[default.deploy]
...
cat > /home/ec2-user/project/kafka-producer/kafka-client.properties << EOF
...
echo "  cd ~/project/kafka-producer"
echo "  ./build-and-run.sh person-data 100 batch-001"
echo ""
echo "To deploy the Lambda function:"
echo "  cd ~/project/msk-dynamodb-processor"
